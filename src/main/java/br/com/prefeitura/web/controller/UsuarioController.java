...
public class UsuarioController {
	...
	public ModelAndView novoCadastro(Model model) {
		...
		LOGGER.info("[LOG-INFO] "+ UsuarioController.class.getSimpleName()+" - CADASTRO DE NOVO USUARIO.");
	...
	/***
	 * CHAMA PÁGINA DE NOTÍCIAS
	 * 
	 * @param model
	 * @return
	 */
	@RequestMapping(value = "/noticia", method = RequestMethod.GET)
	public ModelAndView noticia(Model model) {
...
//	}
	...
	public ModelAndView excluir(@RequestParam("codigoUsuario") Long codigoUsuario){
		...
		this.usuarioService.removeUsuario(codigoUsuario);
	...
	public ModelAndView salvarUsuario(@ModelAttribute @Valid Usuario usuarioModel, final BindingResult result,
		...
		/*
		 * VERIFICA SE TEM ALGUM ERRO (@NotEmpty), SE TIVER ALGUM ERRO DEVEMOS
		 * RETORNAR PARA A MESMA PÁGINA PARA O USUÁRIO CORRIGIR
		 */
		if (result.hasErrors()) {
			 ...
			 * 
			 * SE O SISTEMA ENCONTROU ALGUM ERRO DE VALIDAÇÃO DEVEMOS BUSCAR OS
			 * GRUPOS E MARCAR COMO SELECIONADO NOVAMENTE PARA MOSTRAR NÁ
			 * PÁGINAS DA FORMA QUE O USUÁRIO ENVIO A REQUEST
			 */
			gruposModel.forEach(grupo -> {
				...
				if (usuarioModel.getGrupos() != null && usuarioModel.getGrupos().size() > 0) {
					...
					usuarioModel.getGrupos().forEach(grupoSelecionado -> {
						...
						/*
						 * DEVEMOS MOSTRAR NA PÁGINA OS GRUPOS COM O CHECKBOX
						 * SELECIONADO
						 */
